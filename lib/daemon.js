...
function getProcessMemory(child) {
	return when.promise(function (resolve, reject) {
		childProcess.exec('ps -p' + child.pid + ' -o vsize=',  function (err, stdout, stderr) {
			if (err || stderr) {
				reject(err || stderr);
				return;
			}
			resolve(Math.ceil(parseInt(stdout, 10) / 1000));
		});
	});
}
...
function spawnLambda(config, event) {
	return when.promise(function (resolve, reject) {
		...
		timeoutKill = setTimeout(function () {
			...
			aggrega.counts.timeout = (aggrega.counts.timeout || 0) + 1;
		...
		memoryCheckInterval = setInterval(function () {
			getProcessMemory(child).then(function (mem) {
				if (mem < (metrics.maxMemory || 0)) {
					return;
				}
				
				result.visible.maxMemory = mem;
				if (mem > config.maxMemory) {
					metrics.counts.maxedMemory = (metrics.counts.maxedMemory || 0) + 1;
					aggrega.counts.maxedMemory = (aggrega.counts.maxedMemory || 0) + 1;
