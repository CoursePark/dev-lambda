...
var maxQueueSize = parseInt(process.env.LAMBDADEV_MAX_QUEUE) || 500;
var listenPort = parseInt(process.env.LAMBDADEV_PORT) || 9961;
var logTask = [true, 1, '1', 'true', 'TRUE', false, 0, '0', 'false', 'FALSE'].indexOf(process.env.LAMBDADEV_LOG_TASK) < 5;
...
function spawnLambda(config, event) {
	return when.promise(function (resolve, reject) {
		...
		child.stdout.on('data', function (data) {
			...
			if (logTask) {
				console.log(getFormatedDate(now), config.name, 'output: ' + data.toString());
			}
		});
		child.stderr.on('data', function (data) {
			...
			if (logTask) {
				console.log(getFormatedDate(now), config.name, 'error: ' + data.toString());
			}
		});
			...
			getProcessMemory(child).then(function (mem) {
				...
				periodicMemoryCheck = setInterval(function () {
					getProcessMemory(child).then(function (mem) {
						...
						if (result.maxMemory > config.maxMemory * 1000) {
...
function formatMetrics(data) {
	return {
		counts: (data.counts.total || undefined) && {
			...
			killed: (data.counts.killed.maxMemory || data.counts.killed.timeout || data.counts.killed.queueOverage || undefined) && {
				maxMemory: data.counts.killed.maxMemory,
				timeout: data.counts.killed.timeout,
				queueOverage: data.counts.killed.queueOverage
			}
		},
...
function queueLambda(config, event) {
	if ((aggrega.counts.running || 0) < maxConcurrency) {
	...
	} else if (maxQueueSize > queue.length) {
		...
		var metrics = metricsMap[config.name];
		metrics.counts.killed.queueOverage = (metrics.counts.killed.queueOverage || 0) + 1;
		aggrega.counts.killed.queueOverage = (aggrega.counts.killed.queueOverage || 0) + 1;
	}
}

loadLambdaBaseDir(lambdaBaseDir).then(function () {
	console.log(getFormatedDate(new Date()), 'serving lambdas');
	
	app.get('/', function(req, res) {
		console.log(getFormatedDate(new Date()), 'GET /');
		var output = {};
	...
	_.forOwn(lambdaConfigMap, function (config, name) {
		app.get('/' + config.name, function (req, res) {
			console.log(getFormatedDate(new Date()), 'GET /' + config.name);
	...
	_.forOwn(lambdaConfigMap, function (config, name) {
		app.post('/' + config.name, function (req, res) {
			console.log(getFormatedDate(new Date()), 'POST /' + config.name);
	...
	_.forOwn(lambdaConfigMap, function (config, name) {
		app.post('/sync/' + config.name, function (req, res) {
			console.log(getFormatedDate(new Date()), 'POST /sync/' + config.name);
	...
	app.listen(listenPort).on('error', function (err) {
		if (err.code === 'EADDRINUSE') {
			console.log('ERROR: server address or the port ' + listenPort + ' is already in use');
		}
	});
});
